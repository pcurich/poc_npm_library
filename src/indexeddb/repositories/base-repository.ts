...
export default abstract class BaseRepository<T> implements IRepository<T> {
    ...
    constructor(protected dbContext: IDbContext, protected storeName: string, keyPath: string = "id") {
    ...
    async read(id: IDBValidKey): Promise<T | null> {
        const result = await this.dbContext.runTransaction<T | undefined>(this.storeName, 'readonly', (store) => store.get(id));
